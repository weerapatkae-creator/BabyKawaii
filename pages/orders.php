...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    // If cancelled, restore stock (only if was already deducted)
    if ($newStatus === 'cancelled') {
        $deductedRow = $pdo->prepare("SELECT stock_deducted FROM orders WHERE id=?");
        $deductedRow->execute([$orderId]);
        if ((int)$deductedRow->fetchColumn()) {
            $stmt = $pdo->prepare("SELECT product_id, size, quantity FROM order_items WHERE order_id=?");
            $stmt->execute([$orderId]);
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $item) {
                if (!$item['product_id']) continue;
                $ptRow = $pdo->prepare("SELECT product_type FROM products WHERE id=?");
                $ptRow->execute([$item['product_id']]);
                $ptype = $ptRow->fetchColumn() ?: 'single';
                if ($ptype === 'bundle') {
                    $comps = $pdo->prepare("SELECT product_id, size AS cs, quantity AS cq FROM bundle_items WHERE bundle_id=?");
                    $comps->execute([$item['product_id']]);
                    foreach ($comps->fetchAll(PDO::FETCH_ASSOC) as $c) {
                        $pdo->prepare("UPDATE stock SET quantity = quantity + ? WHERE product_id=? AND size=?")
                            ->execute([$c['cq'] * $item['quantity'], $c['product_id'], $c['cs']]);
                    }
                } else {
                    $pdo->prepare("UPDATE stock SET quantity = quantity + ? WHERE product_id=? AND size=?")
                        ->execute([$item['quantity'], $item['product_id'], $item['size']]);
                }
            }
            $pdo->prepare("UPDATE orders SET stock_deducted=0 WHERE id=?")->execute([$orderId]);
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_order'])) {
    if (!isSuperAdmin()) {
        header('Location: ' . SITE_URL . '/pages/orders.php?err=noperm');
        exit;
    }
    $orderId = (int)$_POST['order_id'];

    // Restore stock if was deducted
    $deductedRow = $pdo->prepare("SELECT stock_deducted FROM orders WHERE id=?");
    $deductedRow->execute([$orderId]);
    if ((int)$deductedRow->fetchColumn()) {
        $stmt = $pdo->prepare("SELECT product_id, size, quantity FROM order_items WHERE order_id=?");
    ...
    header('Location: ' . SITE_URL . '/pages/orders.php?msg=deleted');
    exit;
}
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            ...
                            <?php if (isSuperAdmin()): ?>
                            <button class="btn btn-sm btn-outline-danger ms-1" onclick="confirmDelete(<?= $order['id'] ?>, '<?= htmlspecialchars($order['order_number'], ENT_QUOTES) ?>')" title="ลบออเดอร์">
...
<!-- Delete Order Modal (superadmin only) -->
        ...
        <form method="POST" id="deleteForm">
            ...
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title"><i class="fas fa-trash me-1"></i> ลบออเดอร์</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body text-center">
                    <p class="mb-1">ต้องการลบออเดอร์</p>
                    <p class="fw-bold" id="deleteOrderNum"></p>
                    <p class="text-muted small">สต็อกสินค้าจะถูกคืนกลับโดยอัตโนมัติ</p>
                </div>
...
function confirmDelete(orderId, orderNum) {
    document.getElementById('deleteOrderId').value = orderId;
    document.getElementById('deleteOrderNum').textContent = '#' + orderNum;
