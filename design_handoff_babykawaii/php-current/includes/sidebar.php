<?php
// เมนูหลัก จัดกลุ่มแบบดีไซน์ใหม่ (คง url/page/icon เดิมทุกตัว)
$navGroups = [
    'ภาพรวม' => [
...
// ฟังก์ชันช่วย render badge ของแต่ละเมนู
function renderNavBadge($page, $lowStockCount, $pendingOrders, $unreadMessages) {
    if ($page === 'stock' && $lowStockCount > 0) {
        echo '<span class="nav-badge">' . $lowStockCount . '</span>';
    } elseif ($page === 'orders' && $pendingOrders > 0) {
        echo '<span class="nav-badge">' . $pendingOrders . '</span>';
    } elseif ($page === 'inbox') {
        echo '<span class="nav-badge" id="sidebarInboxBadge" style="' . ($unreadMessages > 0 ? '' : 'display:none') . '">' . ($unreadMessages ?: '') . '</span>';
    }
}
?>
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <img src="<?= SITE_URL ?>/assets/images/logo-light.svg"
             ...
             style="max-width:150px; width:100%; display:block; margin:0 auto 6px;">
    ...
    <nav class="sidebar-nav">
        ...
        <div class="nav-section-label"><?= $groupLabel ?></div>
        <?php foreach ($items as $item): ?>
        <a href="<?= $item['url'] ?>"
           class="nav-item <?= ($currentPage === $item['page']) ? 'active' : '' ?>">
            <i class="fas <?= $item['icon'] ?>"></i>
            <span><?= $item['label'] ?></span>
            <?php renderNavBadge($item['page'], $lowStockCount, $pendingOrders, $unreadMessages); ?>
        </a>
        <?php endforeach; ?>
        <?php endforeach; ?>
